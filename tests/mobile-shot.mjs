...
// 用法：node tests/mobile-shot.mjs [輸出檔名]　預設 tests/_mobile-shot.png
...
const outFile = process.argv[2] || join(repoRoot, "tests", "_mobile-shot.png");
...
function seed() {
  return `(() => {
    const mk=(b,g)=>{const a=[];const e=new Date('2026-06-12');for(let i=0;i<60;i++){const d=new Date(e.getTime()-(59-i)*864e5);a.push({date:d.toISOString().slice(0,10),nav:+(b*(1+i*g)).toFixed(2)});}return a;};
    localStorage.clear();
    localStorage.setItem("fund_tracker_holdings", JSON.stringify([
      {id:"a",assetType:"stock",fundCode:"2330.TW",symbol:"2330.TW",fundName:"台積電",currency:"TWD",isin:"",purchases:[{purchaseId:"p1",type:"LUMP",date:"2026-03-01",units:10,navAtPurchase:900,amount:9000,fee:20,note:""}],dividends:[{divId:"d1",date:"2026-05-10",amount:300,note:""}],rspPlans:[],sells:[{sellId:"s1",date:"2026-05-20",units:4,price:1000,fee:10,note:""}]},
      {id:"b",assetType:"fund",fundCode:"F0X",symbol:"",fundName:"統一奔騰基金",currency:"TWD",isin:"",purchases:[{purchaseId:"p2",type:"RSP",date:"2026-04-01",units:50,navAtPurchase:20,amount:1000,fee:0,note:""}],dividends:[],rspPlans:[],sells:[]},
      {id:"c",assetType:"stock",fundCode:"AAPL",symbol:"AAPL",fundName:"蘋果",currency:"USD",isin:"",purchases:[{purchaseId:"p3",type:"LUMP",date:"2026-02-01",units:5,navAtPurchase:200,amount:1000,fee:0,note:""}],dividends:[],rspPlans:[],sells:[]}
    ]));
    localStorage.setItem("fund_tracker_nav_cache_2330.TW", JSON.stringify({fundCode:"2330.TW",navHistory:mk(900,0.003),source:"t"}));
    localStorage.setItem("fund_tracker_nav_cache_F0X", JSON.stringify({fundCode:"F0X",navHistory:mk(20,0.002),source:"t"}));
    localStorage.setItem("fund_tracker_nav_cache_AAPL", JSON.stringify({fundCode:"AAPL",navHistory:mk(200,0.004),source:"t"}));
...
async function main() {
  ...
  try {
    ...
    await sleep(2500);
    // 選一檔 → 顯示單一資產走勢圖（讓 chartBar 按鈕全部出現）
    await cdp.evaluate(`document.querySelector('#tbody tr[data-id]')?.click()`);
    await sleep(1500);
    ...
    console.log(JSON.stringify({ ok: true, outFile, viewport: dims }));
  } finally {
