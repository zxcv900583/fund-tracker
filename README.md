...
- **基富通報表一鍵匯入＋對帳模式**——「投資總覽 → 匯出報表」的 CSV 直接匯入，自動比對基金級別（含 TISA、好好退休 R 類、外幣級別），整個投資組合 30 秒搬進來；重複匯入＝對帳更新，只補差額不重複記帳；也可匯出成相同格式
- **歷史交易明細匯入（真實 XIRR）**——「交易查詢 → 歷史交易查詢」匯出的交易明細直接匯入：定期定額／轉申購→買入（真實日期與單位數）、轉贖回→賣出（已實現損益與基富通參考損益一致）；與庫存彙總的差額自動生成「期初結轉」，管理彈窗可再用契約參數把結轉拆成逐月扣款（假日順延、總量校正）
...
想要精確的 XIRR 年化，再到「交易查詢 → 歷史交易查詢」匯出交易明細一併匯入（同一顆「CSV」按鈕、自動辨識格式）。查詢區間**上限一年**，可分段選取多次下載；匯入後彙總會被拆成真實日期的逐筆記錄，轉贖回自動記為賣出，與庫存彙總的差額自動生成「期初結轉」——之後重匯投資總覽只做對帳，不會重複記帳。更早的定期定額歷史可在「管理」彈窗用契約參數（成立日／扣款日／金額，見「定期定額查詢/變更」頁）一鍵拆分合成：

![基富通歷史交易匯出位置](基富通歷史交易匯出.png)
